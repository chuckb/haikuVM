 ...
 *
 * what follows is the single base for version 1 and version 2
 *
...
method(main_YLjava_lang_String) {
  initstack(2+(2)+2,    1,    1)    // MaxLocals+(lsp+pc)+MaxStack, purLocals, purParams
  yield(0)
  invokestaticShort(haiku_vm_MicroKernel_clinitHaikuMagic_V)          // 0:    invokestatic    haiku.pc.lib.HaikuMicroKernel.clinitHaikuMagic ()V (11)
  invokestaticShort(haiku_pc_lib_HaikuMicroKernel_init_V)             // 3:    invokestatic    haiku.pc.lib.HaikuMicroKernel.init ()V (12)
  iconst_0()                                                          // 6:    iconst_0
  iconst_0()                                                          // 7:    iconst_0
  invokestaticShort(haiku_vm_MicroKernel_panic_IIV)                   // 8:    invokestatic    haiku.pc.lib.HaikuMicroKernel.panic (II)V (13)
  aload(0)                                                            // 11:   aload_0
  invokestaticShort(arduino_tutorial_JNI_FieldAccess_main_YLjava_lang_String_V)  // 12:   invokestatic    haiku.vm.HaikuMagic.main ([Ljava/lang/String;)V (14)
  goto(29)                                                            // 15:   goto        #29
  yield(18)
  ...
  invokevirtual(MSG_println__Ljava_lang_String__V)                    // 26:   invokevirtual   java.io.PrintStream.println (Ljava/lang/String;)V (17)
  yield(29)
  invokestaticShort(java_lang_Thread_currentThread_Ljava_lang_Thread) // 29:   invokestatic    java.lang.Thread.currentThread ()Ljava/lang/Thread; (18)
  invokevirtual(MSG_stop___V)                                         // 32:   invokevirtual   java.lang.Thread.stop ()V (19)
  return()                                                            // 35:   return
};
 ...
 *
 */
public class Compile2Macros extends HaikuVM implements Aot {
	...
	public Compile2Macros(String classname) {
		super(classname);
	}
	...
	public void compile() throws IOException {
		...
		for (int i=0; i< c.length; i++) {
			...
			if (c[i].getTag()==Constants.CONSTANT_Class) {
				...
				new Compile2Macros(classname).compile();
			}
		}
    ...
    private void includeThis(String include) throws IOException {
        ...
        if (array==null) {
            ...
            PrintOnChange arrayh = new HaikuByteCodeTypes2H(new File(HaikuJava2File.getGenTargetDir()+"/"+include+".h"));
            arrayh.printf("extern const class_t %s__class;\n", array);
            arrayh.close();

            PrintOnChange arrayc = new HaikuByteCode2H(new File(HaikuJava2File.getGenTargetDir()+"/"+include+".c"));
            if (!singleFile) {
	...
	*/
			...
			if (method.isNative()) {
                ...
                if (methodName.endsWith("_clinitHaikuMagic_V")) {
                    ...
                    */
                    ...
                    outc.printf("\n");
                    for (Clinit desc : Clinit.set()) {
                        outc.printf("OPM_INVOKESTATIC(%s__clinit__V)         // -:    invokestatic    %s.<clinit>\n", mangle(desc.getClassName()), desc.getClassName());
                        BC2IDX.put("INVOKESTATIC");
                    }
                    outc.printf("OPM_RETURN()                                               //       return\n");
                    outc.printf("};\n");
                } else {
                    outh.printf("#ifndef %s\n", "native_"+methodName);
		...
		for (int k = 0; k < list.length; k++) {
			...
			if (line.matches("^\\d+:.*")) {
				...
				if (start==0) {
					 ...
					 */
					outc.printf("OPM_method(%s) {\n", methodName);
					outc.printf("    OPM_initstack(%d+(2)+%d, %4d, %4d)    // MaxLocals+(lsp+pc)+MaxStack, purLocals, purParams\n", code.getMaxLocals(), code.getMaxStack(), code.getMaxLocals()-getParams(method), getParams(method));
					if (methodName.endsWith("_clinitHaikuMagic_V")) {
                        conditions.add(Condition.CLINIT.desc("found clinitHaikuMagic() in: "+classname));
						for (Clinit desc : Clinit.set()) {
							totalBClength+=3;
				...
				if (token.length<=2 || !supressPanicSupport(token[2])) {
                    ...
                    outc.println(compileJ2Macro(list[k], bc, start, end, remap));
				}
				start=999;
			} else {
     ...
     * @throws IOException
     */
    private String compileJ2Macro(String jcode, byte bc[], int start, int end, int[] remap) throws IOException {
        String haiku="";
        String[] token= jcode.split("[ \t,]+");
        String ccode="OPM_"+token[1].toUpperCase();

        int len=end-start;
        int index=getConstIndex(jcode);
        switch(len) {
        case 1:
            if (ccode.matches(".+LOAD_\\d") || ccode.matches(".+STORE_\\d")) {
                //OPM_wLOAD_x -> OPM_wLOAD_y
                int local=Integer.parseInt(ccode.substring(ccode.length()-1));
                int rm=remap[local];
                if (local!=rm) {
                    haiku=" !!haiku remap";
                }
                ccode=ccode.replaceFirst("\\d", ""+rm)+"(";
            }
            break;
        case 2:
            if (index>=0) {
                //31:   ldc     0.3010299 (53): OPM_LDC -> OPM_LDC_ext(w
                String ext=getConstExt(index);
                ccode=ccode+"_"+ext+"(";
                if (ext.endsWith("X")) {
                    ccode+=getConstSpec(index, 8);
                } else if (ext.endsWith("C")) {
                    // 18:   ldc        bench.haikuvm.Sync (1)
                    ccode+=mangle("<"+token[2]+">");
                } else {
                    ccode+=getConstSpec(index, 16);
                }
            } else if (token[1].equals("newarray")) {
                //7:    newarray        <byte>
                ccode+="("+size(token[2]);
            } else {
                if (ccode.endsWith("LOAD") || ccode.endsWith("STORE") || token[1].equals("ret")) {
        ...
        case 3:
            ...
            if (token[1].equals("invokevirtual")) {
                ...
                ccode+="("+getMessageParamNo(jcode)+", "+meth;
                break;
            }
...
//                        ccode+=new Formatter().format("NStructADR(%s), ", token[2].replaceFirst("(.*)[.](.+)", "$2"));
                        ...
                        ccode+="_"+btype+new Formatter().format("(NADR(%s), ", token[2].replaceFirst("(.*)[.](.+)", "$2"));
                    }
                } else if (anno.contains("/NativeCVariable")) { //8, 16, 32, 64
                    anno=anno.replaceFirst(".*(NativeCVariable.+);.*", "$1");
                    ObjSpace.set(btype, var, token[1]+"&native");
                    ccode+="_"+btype+"_"+anno+"(NADR("+token[2].replaceFirst("(.*)[.](.+)", "$2")+")";
                } else {
                    ObjSpace.set(btype, var, token[1]);
                    ccode+="_"+btype+"("+mangleField(var);
                }
                break;
            }
            ...
            if (token[1].equals("iinc")) {
                ...
                if (inc==1) {
                    ...
                    ccode+="("+rm+", "+(bc[start+2]&0xff);
                }
                break;
            }
            if (jcode.matches(".+#\\d+$")) {
                String target=jcode.replaceAll(".+#(\\d+)$", "$1");
                ccode+="("+new Formatter().format("%d", Integer.parseInt(target));
                break;
            }
            if (index>=0) {
                if (token[1].startsWith("ldc")) {
                    ...
                    ccode+=new Formatter().format("(%s", adr);
...
//                    ccode+=new Formatter().format("B(%s), ", size(token[2]));
        ...
        case 6:
            if (ccode.equals("OPM_IINC")) {
                ...
                ccode="OPM_WIINC("+ new Formatter().format("%d, %d", rm, (short)((bc[start+4]&0xff)<<8) + (bc[start+5]&0xff));
                break;
            }
            break;
        default:
            if (token[1].equals("invokeinterface")) {
                String meth=getMessageSignature(jcode);
                msg2meth.put(meth, null);
                ccode+=new Formatter().format("(%d, %s, %d, %d", getMessageParamNo(jcode), meth, bc[start+3]&0xff, bc[start+4]&0xff);
                break;
                ...
                ccode+=new Formatter().format("(%s, %d, ", size(token[2]), bc[start+3]&0xff);
            } else {
                for (int i = 1; i < len; i++) {
                    ccode+=(bc[start+i]&0xff)+", ";
                }
            }

        }

        String bcode=ccode.replaceFirst("(?s)OPM_([\\w\\d_]+).*", "$1");
        int p=bcode.indexOf("_Native");
        if (p>0) {
            bcode=bcode.substring(0, p);
        }
        BC2IDX.put(bcode);

        if (ccode.endsWith(", ")) {
            ccode=ccode.substring(0, ccode.length()-2);
        }
        if (ccode.indexOf("(")<0) ccode+="(";
        ccode+=")";
        if(ccode.length()>70) {
...
//                  throw new IndexOutOfBoundsException("int16_t size exceeded by "+((ConstantInteger)cons[i]).getBytes());
	...
	/**
	 * "31:   ldc     0.3010299 (53)" -> 53
	 *
	 * @param jcode
	 * @return constindex or -1
	 */
	private int getConstIndex(String jcode) {
		if (!jcode.matches(".+[(]\\d+[)]$")) return -1;
		return Integer.parseInt(jcode.replaceAll(".+[(](\\d+)[)]$", "$1"));
	}
